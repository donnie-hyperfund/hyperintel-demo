...
export function ImageUploadModeSelector({ disabled }: { disabled: boolean }) {
    ...
    return (
        ...
        >
            <SelectTrigger
                className={`h-auto gap-2 border-none bg-transparent dark:bg-transparent p-0 text-xs text-neutral-400 shadow-none hover:enabled:text-neutral-300 dark:hover:bg-transparent transition-colors focus-visible:ring-0 [&_svg]:text-current disabled:cursor-default disabled:opacity-100 ${
                    shouldShowLoader
                        ? '[&>svg:last-child]:hidden'
                        : '[&>svg:last-child]:transition-transform data-[state=open]:[&>svg:last-child]:rotate-180'
                }`}
            >
                <SelectValue>{LABELS[imageUploadMode]}</SelectValue>
