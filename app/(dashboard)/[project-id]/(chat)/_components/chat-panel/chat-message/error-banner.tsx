...
import { describeError, getUserHint, type RetryAffordance } from '@/lib/errors';
...
export function ErrorBanner({ metadata }: { metadata?: MessageMetadata | null }) {
    ...
    const hint = getUserHint(error?.code);
    ...
    return (
        <div className="mt-3 rounded-2xl border border-red-500/22 bg-red-500/8 p-4">
            <div className="space-y-2">
                ...
                {hint ? (
                    <p className="text-sm leading-6 text-red-100/68">{hint}</p>
                ) : (
