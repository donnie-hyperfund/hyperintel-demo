...
import type { AgentStreamEvent, AgentTool } from '@common/ai/agent';
...
import { z } from 'zod';
import { applyEdits, countLines, type EditOperation, extractViewport } from './document-service';
    ...
    const patchDoc: AgentTool<'patch_document', any, TestToolCtx> = {
        ...
        executor: (input, _ctx, _eCtx, _history, toolCallId) => {
            try {
                ...
                const editOps: EditOperation[] = input.edits.map(
                    (e: { startLine: number; endLine?: number | null; oldContent: string; newContent: string }) => ({
...
describe.skipIf(!API_KEY || process.env.TEST_LLM !== 'true')('Agent document tools (real inference)', () => {
    ...
    it('creates a document: begin → write → finalize', async () => {
        const { stream } = runAgentStream(
            ...
            eCtx,
            baseInput(
                'Create a document called "test-report" with title "Test Report". Write a short paragraph about software testing.',
            ),
            tools,
    ...
    it('searches documents via search_knowledge', async () => {
        ...
        store.upsert(
            'market-analysis.md',
            'Market Analysis',
            'The fintech market grew 25% in Q3. Key players include Stripe and Square.',
            false,
            'Research Report',
        );
    ...
    it('handles multi-step edit with read then patch', async () => {
        ...
        const { stream } = runAgentStream(
            ...
            eCtx,
            baseInput(
                'Edit the "plan" document. In Phase 2, change "Build the MVP with core features." to "Build the MVP with core features and integrations." Also in Phase 3, change "Deploy to production and monitor." to "Deploy to production, monitor, and gather feedback."',
            ),
            tools,
    ...
    it('all tool results have success status (no broken tool calls)', async () => {
        const { stream } = runAgentStream(
            ...
            eCtx,
            baseInput(
                'Create a document named "checklist" with title "QA Checklist" and write a 3-item checklist about code review.',
            ),
            tools,
            { config: { maxToolCalls: 10 } },
        );
