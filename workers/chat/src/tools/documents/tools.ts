...
- For restores: the user has rolled back to a previous version via the version history panel. Use \`read_document\` to read the restored version, then include a \`::document[name]{version=N lines=L}\` directive in your response (using the \`name\`, \`version\`, and \`totalLines\` from the \`read_document\` result) so the artifact indicator appears in the conversation. Briefly acknowledge the restore, note any significant differences from the previous direction, and ask the user how they'd like to proceed.
