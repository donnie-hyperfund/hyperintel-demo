...
## Editing Strategy
- For existing-document patch edits on small/medium documents where \`begin_document\` returns \`content\`, use \`begin_document(mode="edit")\` → \`patch_document\` → \`finalize_document\`. If \`begin_document\` omits \`content\` because the document is over cap, call \`read_document\` for the needed viewport before patching.
...
- Within one \`patch_document\` call, all edit line numbers are interpreted in the same pre-edit frame: the current draft snapshot from \`begin_document\` \`content\`, prior \`touched\`, or \`read_document\`. Batch all edits for that one snapshot into a single \`patch_document\` call when possible.
