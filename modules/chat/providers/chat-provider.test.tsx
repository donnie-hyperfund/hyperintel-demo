...
describe('ChatProvider', () => {
    ...
    it('prepares staged uploads for a new intake chat before send', async () => {
        const replaceStateSpy = vi.spyOn(window.history, 'replaceState');
        apiMock.chats.createIntake.mockResolvedValue({ id: 'company-chat-1' });
        associateUploadsMock.mockResolvedValue(mockResponse({ associatedArtifacts: 1, associatedImages: 1 }));

        const { result } = renderHook(() => useChatContext<'company'>(), { wrapper: companyWrapper });

        await act(async () => {
            await result.current.prepareUploadsForSend({
                stagedArtifactIds: ['artifact-1'],
                imageFileIds: ['file-1'],
            });
        });
        ...
        expect(associateUploadsMock).toHaveBeenCalledWith(
            { artifactIds: ['artifact-1'], imageFileIds: ['file-1'], chatId: 'company-chat-1' },
            'token-abc',
        );
        expect(result.current.chatId).toBe('company-chat-1');
        expect(replaceStateSpy).toHaveBeenCalledWith(null, '', '/companies/company-chat-1');
    ...
    it('skips duplicate upload association when uploads were already prepared', async () => {
        apiMock.chats.createIntake.mockResolvedValue({ id: 'company-chat-1' });
        sendIntakeActionMock.mockResolvedValue(mockResponse());
        ...
        await act(async () => {
            await result.current.sendMessage('intake message', {
                imageFileIds: ['file-1'],
                uploadsAlreadyAssociated: true,
            });
        });

        expect(associateUploadsMock).not.toHaveBeenCalled();
        expect(sendIntakeActionMock).toHaveBeenCalledWith(
            expect.objectContaining({
                message: 'intake message',
                chatId: 'company-chat-1',
    ...
    it('reuses the prepared chat when sending after deferred upload preparation', async () => {
        apiMock.chats.createIntake.mockResolvedValue({ id: 'company-chat-1' });
        associateUploadsMock.mockResolvedValue(mockResponse({ associatedArtifacts: 1, associatedImages: 1 }));
        sendIntakeActionMock.mockResolvedValue(mockResponse());
        ...
        await act(async () => {
            const preparedChatId = await result.current.prepareUploadsForSend({
                stagedArtifactIds: ['artifact-1'],
                imageFileIds: ['file-1'],
            });

            await result.current.sendMessage('intake message', {
                resolvedChatId: preparedChatId,
                imageFileIds: ['file-1'],
                uploadsAlreadyAssociated: true,
            });
        });

        expect(apiMock.chats.createIntake).toHaveBeenCalledTimes(1);
        expect(sendIntakeActionMock).toHaveBeenCalledWith(
            expect.objectContaining({
                message: 'intake message',
                chatId: 'company-chat-1',
