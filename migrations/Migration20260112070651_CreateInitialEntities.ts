...
export class Migration20260112070651_CreateInitialEntities extends Migration {
    override async up(): Promise<void> {
        ...
        this.addSql(
            `alter table "projects" add constraint "projects_fk" foreign key ("user_id") references "users" ("id") on update cascade;`,
        );

        this.addSql(
            `alter table "chats" add constraint "chats_fk" foreign key ("project_id") references "projects" ("id") on update cascade;`,
        );

        this.addSql(
            `alter table "chat_messages" add constraint "chat_messages_fk" foreign key ("chat_id") references "chats" ("id") on update cascade;`,
        );

        this.addSql(
            `alter table "artifacts" add constraint "artifacts_fk" foreign key ("project_id") references "projects" ("id") on update cascade;`,
        );
        this.addSql(
            `alter table "artifacts" add constraint "artifacts_chat_id_fk" foreign key ("chat_id") references "chats" ("id") on update cascade;`,
        );

        this.addSql(
            `alter table "artifact_versions" add constraint "artifact_versions_fk" foreign key ("artifact_id") references "artifacts" ("id") on update cascade;`,
        );
    }
