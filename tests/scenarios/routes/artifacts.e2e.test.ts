...
beforeAll(async () => {
    ...
    const uploaded = em.create(ArtifactEntity, {
        key: 'uploaded.md',
        version: 1,
        project,
        user: userA,
    ...
    const aiArt = em.create(ArtifactEntity, {
        key: 'ai-doc.md',
        version: 1,
        project,
        user: userA,
    ...
    const intake = em.create(ArtifactEntity, { key: 'intake.md', version: 1, user: userA, current_version: null! });
...
describe('artifact list dispatcher', () => {
    ...
    it('GET /api/artifacts?search= — finds unapproved intake artifacts by version title', async () => {
        const { GET } = await import('@/app/api/artifacts/route');
        const { status, body } = await callRoute(GET, '/api/artifacts?search=Intake');
        expect(status).toBe(200);
        expect(body.data.some((a: any) => a.id === intakeArtifactId)).toBe(true);
    });
    ...
    it('GET /api/artifacts?projectId= — returns project artifacts', async () => {
        ...
        expect(status).toBe(200);
        expect(body.data).toBeDefined();
        expect(body.data.some((a: any) => a.id === aiArtifactId)).toBe(true);
    });
...
describe('artifact by ID', () => {
    it('GET /api/artifacts/:id — returns artifact', async () => {
        ...
        const { status, body } = await callRoute(GET, `/api/artifacts/${uploadedArtifactId}`, {
    ...
    it('GET /api/artifacts/:id?version=1 — includes loaded_version', async () => {
        const { GET } = await import('@/app/api/artifacts/[artifactId]/route');
        const { status, body } = await callRoute(GET, `/api/artifacts/${uploadedArtifactId}?version=1`, {
            artifactId: uploadedArtifactId,
        });
        expect(status).toBe(200);
    ...
    it('GET /api/artifacts/:id?version=999 — 404', async () => {
        const { GET } = await import('@/app/api/artifacts/[artifactId]/route');
        const { status } = await callRoute(GET, `/api/artifacts/${uploadedArtifactId}?version=999`, {
            artifactId: uploadedArtifactId,
        });
    ...
    it('GET /api/artifacts/nonexistent — 404', async () => {
        const { GET } = await import('@/app/api/artifacts/[artifactId]/route');
        const { status } = await callRoute(GET, `/api/artifacts/00000000-0000-0000-0000-000000000000`, {
...
describe('artifact delete', () => {
    it('DELETE uploaded artifact → 200', async () => {
        ...
        const { status, body } = await callRoute(
            DELETE,
            `/api/artifacts/${uploadedArtifactId}`,
            { artifactId: uploadedArtifactId },
            { method: 'DELETE' },
        );
        expect(status).toBe(200);
