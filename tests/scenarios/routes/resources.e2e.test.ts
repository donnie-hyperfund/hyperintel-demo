...
beforeAll(async () => {
    ...
    const userRes = em.create(ArtifactEntity, {
        key: 'user-company.md',
        version: 1,
        user: userA,
    ...
    // User-scoped resource with approved version and proposed rename
    const approvedRes = em.create(ArtifactEntity, {
        key: 'approved-persona.md',
        version: 2,
        user: userA,
        current_version: null!,
    });
    const arv1 = em.create(ArtifactVersionEntity, {
        artifact: approvedRes,
        version: 1,
        title: 'Approved Persona',
        content: 'persona content',
        status: 'approved',
        document_type: 'Human Persona',
    });
    const arv2 = em.create(ArtifactVersionEntity, {
        artifact: approvedRes,
        version: 2,
        title: 'Renamed Persona',
        content: 'persona content v2',
        status: 'proposed',
        document_type: 'Human Persona',
    });
    ...
    const importedRes = em.create(ArtifactEntity, {
        key: 'imported-company.md',
        version: 1,
        project,
        user: userA,
        metadata: { importedFrom: 'some-source-id' },
        current_version: null!,
    });
    const irv1 = em.create(ArtifactVersionEntity, {
        artifact: importedRes,
        version: 1,
        title: 'Imported Company',
    ...
    await em.persistAndFlush([userA, userB, project, userRes, urv1, approvedRes, arv1, arv2, importedRes, irv1]);
...
describe('user-scoped resources', () => {
    ...
    it('GET /api/resources?search= — finds proposed resource titles', async () => {
        const { GET } = await import('@/app/api/resources/route');
        const { status, body } = await callRoute(GET, '/api/resources?search=Renamed');
        expect(status).toBe(200);
        expect(body.data.some((a: any) => a.id === approvedResourceId)).toBe(true);
    });
...
describe('resource by key', () => {
    ...
    it('GET /api/resources/:key?version=1 — returns specific version', async () => {
        const { GET } = await import('@/app/api/resources/[key]/route');
        const { status, body } = await callRoute(GET, '/api/resources/user-company.md?version=1', {
            key: 'user-company.md',
        });
        expect(status).toBe(200);
    ...
    it('GET /api/resources/:key?version=999 — 404', async () => {
        const { GET } = await import('@/app/api/resources/[key]/route');
        const { status } = await callRoute(GET, '/api/resources/user-company.md?version=999', {
            key: 'user-company.md',
        });
...
describe('remove project resource', () => {
    ...
    beforeAll(async () => {
        ...
        const res = em.create(ArtifactEntity, {
            ...
            project,
            user: userA,
            metadata: { importedFrom: 'source-123' },
            current_version: null!,
        });
