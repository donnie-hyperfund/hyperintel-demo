 ...
 *
 * Consumes a real SSE response (from fetch or app.request()) and returns
 * parsed StreamEvent[] for assertions in e2e tests.
 *
 * Reuses the app's own parseSSEChunk / parseStreamEventData so tests
 * validate the same parsing logic the frontend uses.
 *
 * Usage:
 *   const res = await fetch(`${BASE}/api/chat`, { method: "POST", body });
 *   const events = await collectStreamEvents(res);
 *
 *   expect(events.some(e => e.type === "done")).toBe(true);
 *   expect(eventsOfType(events, "tool_start")).toHaveLength(0); // redacted
 */

import {
	parseSSEChunk,
	parseStreamEventData,
} from "@/modules/chat/services/stream-parser";
import type {
	StreamEvent,
	StreamEventType,
...
export async function collectStreamEvents(
	...
	if (!body) {
		throw new Error(
			"Response has no body — is the endpoint returning a stream?",
		);
	}
...
export function eventsOfType<T extends StreamEventType>(
	...
	return events.filter(
		(e): e is Extract<StreamEvent, { type: T }> => e.type === type,
	);
}
...
export function expectNoEvents(
	...
	for (const type of types) {
		...
		if (found.length > 0) {
			throw new Error(
				`Expected no "${type}" events but found ${found.length}: ${JSON.stringify(found[0])}`,
			);
		}
	}
}
...
export function expectStreamDone(events: StreamEvent[]) {
	...
	if (!last || last.type !== "done") {
		throw new Error(
			`Expected stream to end with "done" event, got: ${last ? last.type : "no events"}`,
		);
	}
}
